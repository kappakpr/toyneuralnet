...
def main():
    ...
    inputs = np.empty((0,784))
    targets = np.empty([0,3])
    train_inputs = np.empty((0, 784))
    train_targets = np.empty([0,3])
    test_inputs = np.empty((0, 784))
    test_targets = np.empty([0,3])
    ...
    for x in os.listdir():
        if x.endswith(".npy"):
            ...
            x_cat_cls = np.empty([samp_size,3])
            if (cat_class == 0):
                x_cat_cls = np.tile(np.array([0, 0, 0]), samp_size)
            elif(cat_class == 1):
                x_cat_cls = np.tile(np.array([0, 1, 0]), samp_size)
            elif(cat_class == 2):
                x_cat_cls = np.tile(np.array([0, 0, 1]), samp_size)
    ...
    # print(test_targets)
    # print(train_idxs.shape,train_inputs.shape, train_targets.shape, np.unique(train_targets, return_counts=True),test_inputs.shape, test_targets.shape, np.unique(test_targets, return_counts=True))

    p1 = Perceptron(784,3, 3136)

    print("----- training ---", train_inputs.shape[0])
    for i in range(train_inputs.shape[0]):
        if (i % 50 == 0):
            print("--------------- new iteration ------------ ", train_inputs.shape[0] - i)
        p1.train(train_inputs[i]/255, train_targets[i])

    print("After training guesses......")
    for i in range(test_inputs.shape[0]):
        results_a = []
        results_a = p1.guess(test_inputs[i]/255)
        print("Input ",i," target ",np.argmax(test_targets[i]), " ", test_targets[i]," After training guess... ",np.argmax(results_a)," ", results_a)
        x = test_inputs[i].reshape((28, 28))
